...
time. Any ``from demo.scripts.models import ...`` transitively imports ``pynamodb.models`` ->
...
def _setup_dev_env() -> None:
    ...
    os.environ["PYNAMODB_CONFIG"] = os.path.join(
        os.path.dirname(__file__), "..", "pynamodb_settings.py"
    )
